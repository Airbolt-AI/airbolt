...
async function airboltCore(fastify: FastifyInstance, options: CoreOptions): Promise<void> {
  ...
  const config = {
    ...
    ...(options.externalJwtIssuer ? { EXTERNAL_JWT_ISSUER: options.externalJwtIssuer } : {}),
    ...(options.externalJwtPublicKey ? { EXTERNAL_JWT_PUBLIC_KEY: options.externalJwtPublicKey } : {}),
  ...
  const rateLimitOpts = {
    ...
    errorResponseBuilder: (_request: FastifyRequest, context: { max: number; after: string; ttl: number }) => ({
  ...
  fastify.decorate('consumeUserTokens', async function(userId: string, tokens: number) {
  ...
  async function getUserUsage(userId: string) {
    ...
    const usage: any = {
      requests: {
        ...
        remaining: requestStatus ? requestStatus.remainingPoints : config.REQUEST_LIMIT_MAX,
        limit: config.REQUEST_LIMIT_MAX,
        resetAt: new Date(now + (requestStatus?.msBeforeNext || config.REQUEST_LIMIT_TIME_WINDOW)).toISOString(),
      },
    };
    ...
    if (config.TOKEN_LIMIT_MAX && config.TOKEN_LIMIT_MAX > 0) {
      ...
      typedUsage.tokens = {
        ...
        resetAt: new Date(now + (tokenStatus?.msBeforeNext || config.TOKEN_LIMIT_TIME_WINDOW)).toISOString(),
      };
    }
  ...
  fastify.decorate('checkUserRateLimit', async function(request: FastifyRequest, _reply: FastifyReply) {
  ...
  if (config.EXTERNAL_JWT_PUBLIC_KEY || config.EXTERNAL_JWT_SECRET) {
    await fastify.register(fastifyJwt, {
      namespace: 'external',
      secret: config.EXTERNAL_JWT_PUBLIC_KEY || config.EXTERNAL_JWT_SECRET || '',
      verify: {
