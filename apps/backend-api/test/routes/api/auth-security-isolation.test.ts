...
describe('Security Isolation Tests', () => {
  ...
  describe('Internal Token Endpoint Protection', () => {
    ...
    it('should disable tokens in all external auth modes', async () => {
      const authConfigs = [
        // Configured issuer mode
        { EXTERNAL_JWT_ISSUER: 'https://auth.example.com/' },
        // Legacy secret mode
        {
          EXTERNAL_JWT_SECRET: 'shared-secret-for-hmac-validation-testing-only',
        },
      ];
      ...
      for (const config of authConfigs) {
        ...
        app = await build({
          ...config,
          // Ensure we have required environment setup
  ...
  describe('Cross-Mode Security', () => {
    it('should not accept internal tokens when external auth is configured', async () => {
      // First, get an internal token in anonymous mode
      const anonymousApp = await build({
        NODE_ENV: 'production',
        ALLOWED_ORIGIN: 'https://example.com',
        JWT_SECRET:
          'test-secret-for-internal-jwt-generation-in-production-testing-environment',
        OPENAI_API_KEY: 'sk-test-key-for-testing-purposes-only',
        // Explicitly disable external auth
        EXTERNAL_JWT_ISSUER: '',
        EXTERNAL_JWT_PUBLIC_KEY: '',
